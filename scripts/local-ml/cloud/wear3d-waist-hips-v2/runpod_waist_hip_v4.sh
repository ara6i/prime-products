...
WORK_ROOT=/workspace/wear-waist-hips-v4
ARCHIVE_PATH=/workspace/wear-waist-hips-v4-input.tar.gz
LOG_PATH=/workspace/wear-waist-hips-v4-training.log
# A retry on the same paid pod must remain a genuinely fresh run. This exact
# directory contains only disposable V4 job files; teacher/model artifacts are
# already protected in S3 and the new package is downloaded after the cleanup.
rm -rf -- /workspace/wear-waist-hips-v4
...
finish() {
  ...
  if [[ "$exit_code" -ne 0 ]]; then
    ...
    printf '{"schemaVersion":"wear3d-waist-hips-v4-failure/v1","jobId":"%s","state":"failed","exitCode":%s,"teacherInputsReadOnly":true,"previousWeightsUsed":false,"sealed448SubjectsUsedForTraining":0}\n' \
...
PYTHONPATH="$WORK_ROOT/code" python -m unittest \
  ...
  "$WORK_ROOT/code/waist_hip_student_v4_test.py" -v

echo 'Starting mandatory ten-person overfit proof'
timeout --signal=TERM --kill-after=60 1800 \
  env PYTHONPATH="$WORK_ROOT/code" python "$WORK_ROOT/code/train_waist_hip_v4.py" \
  ...
  --decode-workers 20 \
  --max-wall-minutes 24
PYTHONPATH="$WORK_ROOT/code" python "$WORK_ROOT/code/verify_overfit_v4.py" \
...
echo 'Overfit proof passed; starting full sealed training'
...
env PYTHONPATH="$WORK_ROOT/code" python "$WORK_ROOT/code/export_waist_hip_v4_onnx.py" \
  ...
  --model-version "wear3d-waist-hips-v4-${WEAR_JOB_ID}"
...
upload_log
echo 'WEAR_V4_COMPLETE_AND_READY_TO_STOP'

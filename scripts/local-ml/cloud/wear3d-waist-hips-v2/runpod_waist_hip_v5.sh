...
WORK_ROOT=/workspace/wear-waist-hips-v5
ARCHIVE_PATH=/workspace/wear-waist-hips-v5-input.tar.gz
LOG_PATH=/workspace/wear-waist-hips-v5-training.log
# This exact path contains only disposable V5 job files. Immutable inputs and
# completed artifacts live in S3, so a retry must start with no model weights.
rm -rf -- /workspace/wear-waist-hips-v5
...
finish() {
  ...
  if [[ "$exit_code" -ne 0 ]]; then
    ...
    printf '{"schemaVersion":"wear3d-waist-hips-v5-failure/v1","jobId":"%s","state":"failed","exitCode":%s,"teacherInputsReadOnly":true,"previousWeightsUsed":false,"sealed448SubjectsUsedForTraining":0}\n' \
...
PYTHONPATH="$WORK_ROOT/code" python -m unittest \
  ...
  "$WORK_ROOT/code/waist_hip_student_v5_test.py" -v

echo 'Starting mandatory ten-person V5 overfit proof'
timeout --signal=TERM --kill-after=60 2100 \
  env PYTHONPATH="$WORK_ROOT/code" python "$WORK_ROOT/code/train_waist_hip_v5.py" \
  ...
  --decode-workers 20 \
  --max-wall-minutes 29
PYTHONPATH="$WORK_ROOT/code" python "$WORK_ROOT/code/verify_overfit_v5.py" \
...
echo 'V5 overfit proof passed; starting full sealed 3451/427 training'
...
env PYTHONPATH="$WORK_ROOT/code" python "$WORK_ROOT/code/export_waist_hip_v5_onnx.py" \
  ...
  --model-version "wear3d-waist-hips-v5-${WEAR_JOB_ID}"
...
upload_log
echo 'WEAR_V5_COMPLETE_AND_READY_TO_STOP'

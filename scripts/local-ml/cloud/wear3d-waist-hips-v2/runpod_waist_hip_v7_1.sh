...
WORK_ROOT=/workspace/wear-waist-hips-v7-1
ARCHIVE_PATH=/workspace/wear-waist-hips-v7-1-input.tar.gz
LOG_PATH=/workspace/wear-waist-hips-v7-1-training.log
rm -rf -- "$WORK_ROOT"
...
finish() {
  ...
  if [[ "$exit_code" -ne 0 ]]; then
    ...
    printf '{"schemaVersion":"wear3d-waist-hips-v7-1-failure/v1","jobId":"%s","state":"failed","exitCode":%s,"teacherInputsReadOnly":true,"previousWeightsUsed":false,"sealed448SubjectsUsedForTraining":0}\n' \
...
PYTHONPATH="$WORK_ROOT/code" python -m unittest \
  ...
  "$WORK_ROOT/code/waist_hip_student_v7_1_test.py" -v

echo 'Starting mandatory ten-person V7.1 overfit proof'
timeout --signal=TERM --kill-after=60 2400 \
  env PYTHONPATH="$WORK_ROOT/code" python "$WORK_ROOT/code/train_waist_hip_v7_1.py" \
  ...
  --decode-workers 20 \
  --max-wall-minutes 34
PYTHONPATH="$WORK_ROOT/code" python "$WORK_ROOT/code/verify_overfit_v7_1.py" \
...
echo 'V7.1 proof passed; starting fresh 3451/427 canonical training'
...
env PYTHONPATH="$WORK_ROOT/code" python "$WORK_ROOT/code/export_waist_hip_v7_1_onnx.py" \
  ...
  --model-version "wear3d-waist-hips-v7-1-${WEAR_JOB_ID}"
...
upload_log
echo 'WEAR_V7_1_COMPLETE_AND_READY_TO_STOP'

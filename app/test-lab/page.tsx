...
import { Activity, ArrowRight, Gauge, PanelsTopLeft, Ruler } from "lucide-react";
...
const LABS = [
  {
    title: "Try-On Test",
    description: "Run the SDK virtual try-on flow against local and staging data.",
    href: "/try-on-test",
    icon: Activity,
  },
  {
    title: "AI Sizing Lab",
    description: "Debug size-chart parsing, measurements, and recommendation output.",
    href: "/try-on-test/sizing-lab",
    icon: Ruler,
  },
  {
    title: "Capacity Lab",
    description: "Check capacity routes, scenarios, and request safety before testing load.",
    href: "/try-on-test/capacity-lab",
    icon: Gauge,
  },
  {
    title: "PDP Studio",
    description: "Draft PDP prompts with a photo input and product cloth image.",
    ...
    <main className="min-h-screen bg-[#f6f8fc] px-5 py-10 text-text-primary">
      <section className="mx-auto w-full max-w-5xl">
        <div className="rounded-[28px] border border-[#dbe4f4] bg-white p-6 shadow-[0_24px_80px_rgba(33,84,239,0.08)] sm:p-8">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-blue">Local tools</p>
          ...
          <p className="mt-4 max-w-2xl text-base leading-7 text-text-body">
            Internal test routes for try-on, sizing, and capacity checks.
          </p>
            ...
            {LABS.map((lab) => {
              ...
              return (
                ...
                >
                  ...
                  <h2 className="mt-5 text-xl font-semibold tracking-[-0.03em] text-[#0d1324]">{lab.title}</h2>
                  <p className="mt-3 flex-1 text-sm leading-6 text-text-body">{lab.description}</p>

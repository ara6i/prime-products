...
import {
  getRawProductDetail,
  getStaticProductIds,
} from "../../product/services/productDetail.service";
...
const products = brandCatalogData.flatMap((catalog) =>
  catalog.products.map((product) => ({ catalog, product })),
);
...
describe("Brand product photography", () => {
  ...
  it.each(products)(
    "keeps $product.id matched from catalog to PDP",
    async ({ catalog, product }) => {
      const original = importedBrandProducts[catalog.id].find(
        ({ id }) => id === product.id,
      )!;
      const source = await getRawProductDetail(product.id);
      expect(source?.kind).toBe("brand");
      expect(
        getStaticProductIds().filter((id) => id === product.id),
      ).toHaveLength(1);
      ...
      expect(product.image).toBe(
        `/media/global-shop/brand-products-v1/${product.id}-front.png`,
      );
      ...
      expect(
        pdp.information.find(({ id }) => id === "materials")?.summary,
      ).toContain("has not been supplied");
      ...
      expect(
        pdp.related.every((item) =>
          catalog.products.some(
            (candidate) =>
              candidate.id === item.id && candidate.image === item.image,
          ),
        ),
      ).toBe(true);
    },
  ...
  it("uses a distinct generated file for every SKU", () => {
    const hashes = products.map(({ product }) =>
      createHash("sha256")
        .update(readFileSync(assetFile(product.image)))
        .digest("hex"),
    );
  ...
  it("corrects imported neckline conflicts and the top filed under bottoms", () => {
    const zenana = products.find(
      ({ product }) => product.id === "zenana-08",
    )!.product;
    expect(zenana.name).not.toContain("V-Neck");
    expect(zenana.description).toContain("round neckline");
    expect(
      products.find(({ product }) => product.id === "davi-dani-06")!.product
        .category,
    ).toBe("Tops");
    ...
    const bombom = products.find(
      ({ product }) => product.id === "bombom-04",
    )!.product;
  ...
  it.each(brandCatalogData)(
    "keeps $name editorial and category cards tied to the same catalog photos",
    (catalog) => {
      const editorial = mapBrandCatalogToEditorial(catalog);
      expect(
        editorial.droppedProducts.map(({ product }) => product.image),
      ).toEqual(catalog.products.slice(0, 4).map(({ image }) => image));
      for (const card of editorial.categoryCards) {
        expect(
          catalog.products.some(
            (product) =>
              product.image === card.image &&
              (!card.value || product.category === card.value),
          ),
        ).toBe(true);
      }
    },

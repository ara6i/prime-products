...
let cache: FeatureCollection<Geometry, CountryFeatureProperties> | null = null;

// Minimal ISO 3166-1 numeric → alpha-2 lookup for countries likely to appear
// in merchant data. Falls back to the numeric string if unknown.
...
export function numericToAlpha2(numeric: string | null | undefined): string | null {
  if (!numeric) return null;
  return numericToIso2[String(numeric).padStart(3, "0")] ?? null;
}
...
> {
  ...
  const file = path.join(process.cwd(), "public", "data", "world-110m.json");
  const raw = await fs.readFile(file, "utf8");
  const topo = JSON.parse(raw) as Parameters<typeof feature>[0];
  const obj = topo.objects.countries;
  const collection = feature(topo, obj) as unknown as FeatureCollection<
    Geometry,
    { name: string }
  >;

  const withNumeric: FeatureCollection<Geometry, CountryFeatureProperties> = {
    ...
    features: collection.features.map((f) => {
      ...
      } as typeof f & { properties: CountryFeatureProperties };
    }),
  };

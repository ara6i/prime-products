...
import { isTeacherProofScan } from "@/app/try-on-test/wear-teacher-proof/teacherProof.server";
import { teacherBlenderDirectory } from "@/app/try-on-test/wear-teacher-proof/teacherBlender.server";
...
export async function GET(request: Request) {
  if (!isTestLabAvailableForHost(request.headers.get("host"))) {
    return NextResponse.json({ error: "WEAR teacher artifacts are private Test Lab only." }, { status: 403 });
  }
  const parameters = new URL(request.url).searchParams;
  const scanId = parameters.get("scanId")?.toUpperCase() ?? "";
  const kind = parameters.get("kind") as ArtifactKind | null;
  if (!kind || !Object.hasOwn(ARTIFACTS, kind) || !(await isTeacherProofScan(scanId))) {
    return NextResponse.json({ error: "Unknown private WEAR teacher artifact." }, { status: 404 });
  }
  const definition = ARTIFACTS[kind];
  try {
    const file = await readFile(path.join(teacherBlenderDirectory(scanId), definition.fileName));
    ...
    return NextResponse.json({ error: "Render this teacher scan first." }, { status: 404 });
  }
}

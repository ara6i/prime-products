...
const DEFAULT_CHECKPOINT_ROOT = ".local-ml/checkpoints/wear3d-fresh-v1-full-runpod-h100-20260824";
...
interface FreshRuntimeManifest {
  schemaVersion: "wear3d-fresh-onnx-runtime/v1";
  modelVersion: string;
  modelSha256: string;
...
function checkpointRoot() {
  const configured = process.env.WEAR_FRESH_MODEL_DIR?.trim();
...
async function loadRuntimeManifest() {
  ...
  if (
    manifest.schemaVersion !== "wear3d-fresh-onnx-runtime/v1"
    ...
    || manifest.targetCount !== 371
    || manifest.targetSchema?.length !== 371
    || manifest.outputsPhysicalValues !== true
    || manifest.sealedTestSubjectsUsed !== 0
  ) {
    throw new Error("The fresh local ONNX package is incomplete or incompatible.");
  }
...
function warningsForPhoto(body: FreshPredictBody, mask: CanonicalMask) {
  ...
  if (Array.isArray(body.landmarks) && body.landmarks.length === 33) {
    ...
    if (
      ...
      warnings.push("At least one hand is raised; the fresh model learned a neutral standing A-pose.");
    }
  }
  return warnings;
}
...
function responseError(error: unknown, status = 400) {
  return NextResponse.json({
    ok: false,
    error: error instanceof Error ? error.message : "Fresh ONNX inference failed.",
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      syntheticWearValidated: manifest.syntheticWearValidated,
      realPhotoValidated: manifest.realPhotoValidated,
      sdkReady: manifest.sdkReady,
      sealedTestSubjectsUsed: manifest.sealedTestSubjectsUsed,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!targetTensor || targetTensor.data.length !== manifest.targetCount) {
      throw new Error("The fresh model returned an incompatible target tensor.");
    }
    ...
    const rows = ROWS.map(([kind, label, color]) => {
      ...
      return {
        ...
        syntheticValidation: manifest.validationMetrics.rows?.[kind] ?? null,
      };
    });
    ...
    return NextResponse.json({
      ...
      inputContract: {
        ...
        cameraHandling: "The fresh ONNX predicts WEAR-taught yaw, pitch, roll, lens and distance corrections as outputs.",
      },
      profile: {

    ...
    <section id="introduction">
      ...
      <div className="rounded-xl border border-[#2154EF]/20 bg-[#2154EF]/5 p-5 mb-8">
        <p className="text-sm text-gray-800 leading-relaxed">
          <strong className="font-semibold">Profiles live in localStorage on the
          user&apos;s browser.</strong> PrimeStyle does not store user profiles or
          measurements on our servers, and there is no backend account system for end
          users. You do not need a database to use any of this.
        </p>
      </div>
...
const AUTH_CURL = `curl -X POST https://myaifitting.com/api/v1/tryon \\
...
const TRYON_CREATE_CURL = `curl -X POST https://myaifitting.com/api/v1/tryon \\
...
res = requests.post(
    "https://myaifitting.com/api/v1/tryon",
...
const TRYON_STATUS_CURL = `curl https://myaifitting.com/api/v1/tryon/status/65c8d1a4e2b4f5d3c8a91234 \\
  -H "Authorization: Bearer $PRIMESTYLE_API_KEY"`;

const TRYON_STATUS_NODE = `const jobRes = await fetch(
  \`https://myaifitting.com/api/v1/tryon/status/\${galleryId}\`,
...
const TRYON_STATUS_PYTHON = `res = requests.get(
    f"https://myaifitting.com/api/v1/tryon/status/{gallery_id}",
...
const TRYON_STREAM_CURL = `curl -N "https://myaifitting.com/api/v1/tryon/stream?key=$PRIMESTYLE_API_KEY"`;

const TRYON_STREAM_JS = `const es = new EventSource(
  \`https://myaifitting.com/api/v1/tryon/stream?key=\${apiKey}\`
);
...
const SIZING_RECOMMEND_CURL = `curl -X POST https://myaifitting.com/api/v1/sizing/recommend \\
  -H "Authorization: Bearer $PRIMESTYLE_API_KEY" \\
  -H "Content-Type: application/json" \\
  -d '{
    "method": "exact",
...
const SIZING_RECOMMEND_NODE = `const res = await fetch(
  "https://myaifitting.com/api/v1/sizing/recommend",
  {
    method: "POST",
    headers: {
      Authorization: \`Bearer \${apiKey}\`,
...
const SIZING_ESTIMATE_NODE = `const res = await fetch(
  "https://myaifitting.com/api/v1/sizing/estimate",
  {
    method: "POST",
    headers: {
      Authorization: \`Bearer \${apiKey}\`,
...
const GUEST_TRYON_CURL = `curl -X POST https://myaifitting.com/api/public/tryon \\
  -H "Content-Type: application/json" \\
  -d '{
    "modelImage": "https://cdn.example.com/user.jpg",
    "garmentImage": "https://cdn.example.com/shirt.jpg"
...
const SDK_WEB_COMPONENT = `<script type="module" src="https://unpkg.com/@primestyleai/tryon"></script>

<primestyle-tryon
  product-image="https://cdn.example.com/products/shirt.jpg"
  product-title="Linen Oxford Shirt"
  product-id="sku-12345"
  api-url="https://myaifitting.com"
      ...
      <div id="sdk-component" className="scroll-mt-14 lg:scroll-mt-24 mt-10">
        ...
        <ParamTable
          ...
          params={[
            ...
            { name: "apiUrl", type: "string", required: false, description: "PrimeStyle API base URL. Defaults to NEXT_PUBLIC_PRIMESTYLE_API_URL or https://myaifitting.com." },
        ...
        <div className="rounded-xl border border-[#2154EF]/20 bg-[#2154EF]/5 p-5 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-700 leading-relaxed">
              ...
              parsing, per-variant recommendations, and zero-code install is on the
              roadmap. Until then, you can integrate via the React component or the
              raw HTTP API.
            </p>
          </div>
        </div>
  ...
  return (
    <section id="support">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <a
          href="https://www.myaifitting.com/developer/dashboard/billing"
          className="rounded-xl border border-gray-200 bg-[#F9FAFB] p-6 hover:border-[#2154EF]/30 transition-colors group"
        >
          <h3 className="font-semibold text-gray-900 group-hover:text-[#2154EF] transition-colors mb-2">
            Enterprise
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Profiles live in the user&apos;s browser localStorage. PrimeStyle does
              not offer a server-side profile store and no endpoint for profile CRUD.
              If you need profiles to follow a user across devices, you are free to
              mirror them into your own backend.
            </p>
          </div>
          <div>

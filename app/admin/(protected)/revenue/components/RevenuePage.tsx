...
function MetricCard({
  ...
  return (
    <article className="rounded-[var(--radius-customer-card)] border border-customer-border bg-customer-card p-5">
      <p className="text-xs font-semibold uppercase tracking-[0.12em] text-customer-muted">
        {label}
      </p>
      <p className={`mt-3 text-3xl font-semibold leading-none ${valueClass}`}>
        {value}
      </p>
...
function EmptyPanel({ title, body }: { title: string; body: string }) {
  return (
    <div className="rounded-[var(--radius-customer-card)] border border-dashed border-customer-border bg-customer-card p-8 text-center">
      ...
      <p className="mx-auto mt-2 max-w-2xl text-sm leading-relaxed text-text-body">
        {body}
      </p>
    </div>
  );
      ...
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            ...
            <span className="mt-1 block text-sm text-customer-muted">
              {tab.description}
            </span>
          </Link>
        );
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-text-body">
            Where Shopify monthly revenue is coming from.
          </p>
        </div>
        <p className="text-sm font-semibold text-brand-blue">
          {formatCurrency(total, currency)} MRR
        </p>
      </div>
          ...
          <div key={item.label}>
            ...
            <div className="h-3 overflow-hidden rounded-full bg-customer-soft">
              <div
                ...
                style={{ width: `${percent(item.amount, total)}%` }}
              />
            </div>
          </div>
        ))}
...
function MonthlyChart({
  ...
  return (
    <section className="rounded-[var(--radius-customer-card)] border border-customer-border bg-customer-card p-5">
      ...
      <p className="mt-1 text-sm text-text-body">
        New paid Shopify stores by install month, using current plan value.
      </p>

      <div className="mt-6 grid grid-cols-6 gap-3">
        {months.map((month) => {
          const barHeight =
            month.mrr > 0 ? Math.max(8, (month.mrr / maxMrr) * 100) : 2;

          return (
            <div key={month.month} className="min-w-0">
              <div className="flex h-36 items-end rounded-xl bg-customer-soft px-2 pb-2">
                <div
                  className="w-full rounded-t-lg bg-brand-blue"
              ...
              <p className="mt-1 truncate text-center text-xs text-customer-muted">
                {formatCurrency(month.mrr, currency)}
              </p>
            </div>
          );
        })}
...
function PlanBreakdown({
  ...
  return (
    <section className="rounded-[var(--radius-customer-card)] border border-customer-border bg-customer-card p-5">
      ...
      <p className="mt-1 text-sm text-text-body">
        Revenue grouped by the selected product and try-on package.
      </p>
      ...
      {rows.length > 0 ? (
        <div className="mt-6 space-y-4">
          {rows.map((row) => (
            <div
              key={row.label}
              className="border-b border-customer-border pb-4 last:border-b-0 last:pb-0"
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="font-semibold text-text-primary">{row.label}</p>
                  <p className="mt-1 text-sm text-customer-muted">
                    {formatNumber(row.stores)} stores ·{" "}
                    {formatNumber(row.products)} products ·{" "}
                    {formatNumber(row.tryOns)} try-ons
                  </p>
                </div>
              ...
              <div className="mt-3 h-2 overflow-hidden rounded-full bg-customer-soft">
                <div
                  className="h-full rounded-full bg-brand-blue"
                  style={{ width: `${percent(row.mrr, maxMrr)}%` }}
                />
              </div>
            </div>
          ))}
        ...
        <p className="mt-6 rounded-xl bg-customer-soft px-4 py-5 text-sm text-text-body">
          No paid Shopify plans yet.
        </p>
      )}
    </section>
  );
    ...
    <section className="overflow-hidden rounded-[var(--radius-customer-card)] border border-customer-border bg-customer-card">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-customer-border p-5">
        <div>
          ...
          <p className="mt-1 text-sm text-text-body">
            Highest current monthly Shopify billing records.
          </p>
        </div>
      ...
      {stores.length > 0 ? (
            ...
            <table className="w-full min-w-[920px] text-left text-sm">
              ...
              <tbody>
                {stores.map((store) => (
                  <tr
                    key={store.id}
                    className="border-t border-customer-border"
                  >
                    ...
                    <td className="px-5 py-4 font-semibold text-text-primary">
                      {formatCurrency(store.monthlyRevenue, currency)}
                    </td>
                    <td className="px-5 py-4 text-text-body">
                      <p>
                        {formatNumber(store.selectedProductCount ?? 0)} products
                      </p>
                      <p className="mt-1 text-xs text-customer-muted">
                        Platform {formatCurrency(store.platformFee, currency)}
                      </p>
                    </td>
                    <td className="px-5 py-4 text-text-body">
                      <p>
                        {formatNumber(store.tryOnPackageQuantity ?? 0)} included
                      </p>
                      <p className="mt-1 text-xs text-customer-muted">
                        {formatRate(store.effectiveTryOnRate, currency)}
                      </p>
                    </td>
                    <td className="px-5 py-4">
                      <span className="rounded-full bg-customer-soft px-3 py-1 text-xs font-semibold text-text-body">
                    ...
                    <td className="px-5 py-4 text-text-body">
                      {formatDate(store.currentPeriodEnd)}
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              <article key={store.id} className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <h4 className="break-words font-semibold text-text-primary">
                      {store.shopName || store.shopDomain}
                    </h4>
                    ...
                    {store.ownerEmail ? (
                      <p className="mt-1 break-all text-xs text-text-body">
                        {store.ownerEmail}
                      </p>
                    ) : null}
                  </div>
                  <span className="shrink-0 rounded-full bg-customer-soft px-3 py-1 text-xs font-semibold text-text-body">
                    {store.billingTest ? "Test" : "Live"}
                  </span>
                </div>
                ...
                <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 border-t border-customer-border pt-4 text-sm">
                  <div>
                    <dt className="text-xs text-customer-muted">Monthly</dt>
                    <dd className="mt-1 font-semibold text-text-primary">
                      {formatCurrency(store.monthlyRevenue, currency)}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs text-customer-muted">Period end</dt>
                    <dd className="mt-1 text-text-body">
                      {formatDate(store.currentPeriodEnd)}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs text-customer-muted">Plan</dt>
                    <dd className="mt-1 text-text-body">
                      {formatNumber(store.selectedProductCount ?? 0)} products
                    </dd>
                    <dd className="mt-1 text-xs text-customer-muted">
                      Platform {formatCurrency(store.platformFee, currency)}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs text-customer-muted">Try-ons</dt>
                    <dd className="mt-1 text-text-body">
                      {formatNumber(store.tryOnPackageQuantity ?? 0)} included
                    </dd>
                    <dd className="mt-1 text-xs text-customer-muted">
        ...
        <div className="p-5">
          <p className="rounded-xl bg-customer-soft px-4 py-5 text-sm text-text-body">
            No active paid Shopify stores yet.
          </p>
        </div>
      )}
    </section>
...
function OperationsSnapshot({ report }: { report: ShopifyRevenueReport }) {
  ...
  return (
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
      <MetricCard
        label="Included try-ons"
        value={formatNumber(summary.includedTryOns)}
        helper="Recurring allowance across paid plans."
      />
      <MetricCard
        label="Selected products"
        value={formatNumber(summary.selectedProducts)}
        helper="Products enabled for Shopify billing."
      />
      <MetricCard
        label="Try-ons used"
        value={formatNumber(summary.tryOnsUsed)}
        helper="Usage recorded across active Shopify stores."
      />
      <MetricCard
        label="Try-on balance"
        value={formatNumber(summary.tryOnBalance)}
        helper="Remaining usage tracked on active stores."
      />
      <MetricCard
        label="Scheduled MRR"
        value={formatCurrency(summary.scheduledMrr, report.currency)}
        helper="Plan changes queued for next cycle."
      />
    </section>
  );
}
...
function ShopifySummary({ report }: { report: ShopifyRevenueReport }) {
  ...
  return (
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      <MetricCard
        label="Shopify MRR"
        value={formatCurrency(summary.totalMrr, currency)}
        helper="Current active paid Shopify plans."
        tone="blue"
      />
      <MetricCard
        label="Live MRR"
        value={formatCurrency(summary.liveMrr, currency)}
        helper="Real Shopify billing, excluding test charges."
        tone="green"
      />
      <MetricCard
        label="Test MRR"
        value={formatCurrency(summary.testMrr, currency)}
        helper="Staging and Shopify test billing records."
      />
      <MetricCard
        label="Annual run rate"
        value={formatCurrency(summary.annualRunRate, currency)}
        helper="Monthly recurring revenue multiplied by 12."
      />
      <MetricCard
        label="Paid stores"
      ...
      <MetricCard
        label="Average MRR"
        value={formatCurrency(summary.averageMrrPerPaidStore, currency)}
        helper="Average revenue per paid Shopify store."
      />
    </section>
  );
}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <RevenueMixChart
          items={report.composition}
          total={report.summary.totalMrr}
          currency={report.currency}
        />
        <MonthlyChart
          months={report.monthlySeries}
          currency={report.currency}
        />
      </div>
  ...
  return (
    <div className="space-y-5">
      <section className="grid gap-4 md:grid-cols-3">
        ...
        />
        <MetricCard
          label="Paid Shopify stores"
          value={formatNumber(report.summary.activePaidStores)}
          helper="Stores with an active approved billing plan."
        />
        <MetricCard
          label="SDK revenue"
          value="Coming soon"
          helper="Reserved for direct SDK subscriptions and invoices."
        />
      </section>
      ...
      <section className="rounded-[var(--radius-customer-card)] border border-customer-border bg-customer-card p-5">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-text-body">
          This report currently uses Shopify billing records. The SDK tab is
          ready for the direct SDK revenue source when those invoices are
          connected.
        </p>
      </section>
...
export function RevenuePage({ activeView, shopifyReport }: RevenuePageProps) {
  const updatedLabel = formatDate(
    shopifyReport.lastUpdatedAt || shopifyReport.generatedAt,
  );
    ...
    <section className="mx-auto max-w-[1500px] space-y-6">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-relaxed text-text-body">
            Clean revenue reporting for Shopify billing today, with room for SDK
            revenue next.
          </p>
        </div>
      ...
      {activeView === "shopify" ? (
        <ShopifyReport report={shopifyReport} />
      ) : null}

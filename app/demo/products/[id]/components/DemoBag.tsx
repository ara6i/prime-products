...
function sizeSummary(payload: DemoAddToBagPayload, fallback: string): string {
  ...
  return payload.recommendedSize?.trim() || fallback;
}
...
export function buildDemoBagItem({
  ...
  color,
  payload,
  suggestedSizeLabel = "Suggested size",
}: {
  ...
  const sizeLabel = sizeSummary(payload, suggestedSizeLabel);
...
export function DemoBagButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      aria-label={translateDemo("openBag", { count, plural: count === 1 ? "" : "s" })}
...
export function DemoBagDrawer({
  ...
  return (
    <div className={`fixed inset-0 z-50 ${open ? "pointer-events-auto" : "pointer-events-none"}`}>
      <button
        type="button"
        aria-label={translateDemo("closeBag")}
        onClick={onClose}
      ...
      >
        <div className={`flex h-full flex-col ${isMobile ? "max-h-[84vh]" : ""}`}>
          <div className="flex items-center justify-between border-b border-border-light px-5 py-4">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-brand-blue">{translateDemo("bag")}</p>
              <h2 className="mt-1 text-lg font-semibold text-text-primary">{translateDemo("yourSelections")}</h2>
            </div>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
            {items.length === 0 ? (
              <div className="flex min-h-[220px] flex-col items-center justify-center text-center">
                ...
                <p className="mt-4 text-sm font-semibold text-text-primary">{translateDemo("yourBagEmpty")}</p>
                <p className="mt-1 max-w-[220px] text-xs leading-5 text-text-hint">
                  {translateDemo("addSuggestedFit")}
                </p>
              </div>
            ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <button
                          ...
                          aria-label={translateDemo("removeItem", { title: item.title })}
                        >
                      ...
                      <div className="mt-3 flex items-center justify-between">
                        <div className="inline-flex items-center rounded-full border border-border-light bg-white">
                          <button
                            ...
                            aria-label={translateDemo("decreaseQuantity", { title: item.title })}
                          >
                          ...
                          <button
                            ...
                            aria-label={translateDemo("increaseQuantity", { title: item.title })}
                          >
                            <Plus className="h-3.5 w-3.5" />
                          </button>
                        </div>
                        <span className="text-[11px] font-medium text-text-caption">
                          {item.price !== null && item.price > 0 ? formatPrice(item.price * item.quantity) : translateDemo("noCheckout")}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="border-t border-border-light px-5 py-4">
            <div className="flex items-center justify-between">
              <span className="text-sm font-semibold text-text-primary">
                {translateDemo("itemCount", { count: totalCount, plural: totalCount === 1 ? "" : "s" })}
              </span>
              <button
                type="button"
                onClick={onClear}

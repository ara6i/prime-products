  ...
  return (
    <div className={styles.dialogBackdrop} onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby="merchant-interest-title">
        ...
        <span>PrimeStyleAI Shopping Network</span><h2 id="merchant-interest-title">{viewModel.interest.title}</h2><p>{viewModel.interest.body}</p>
        {submissionState === "success" ? <div className={styles.dialogSuccess}><CheckCircle size={36} weight="fill" /><strong>{message}</strong><button type="button" onClick={onClose}>Close</button></div> : (
          <form onSubmit={handleSubmit}>
            <label>Name<input ref={nameInputRef} name="name" autoComplete="name" required placeholder="Your name" /></label>
            ...
            <button type="submit" disabled={submissionState === "submitting"}>{submissionState === "submitting" ? "Joining…" : "Join the network"}<ArrowRight size={17} /></button>
          </form>
        )}
      </section>
    </div>

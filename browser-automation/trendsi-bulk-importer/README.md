...
- Manager and worker tabs are marked non-discardable so Chrome Memory Saver cannot silently suspend the importer.
- Each worker owns one fixed page range, so two workers never intentionally process the same catalog page.
...
- Each confirmed page records its Trendsi product IDs, selected count, and submission time.
- The single submission lock stays held until every selected product shows its Shopify icon. The worker displays both import progress and confirmed-icon progress while it waits.
